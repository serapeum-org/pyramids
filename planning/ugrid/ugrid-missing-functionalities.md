...
- Every task ends **green**: `pixi run test` (or `pytest tests/ugrid`) passes, `pre-commit` clean, new
  public symbols documented (Google docstrings + a `docs/reference/netcdf/ugrid/*.md` stub if a new
  module/class is added), and a `docs/change-log.md` entry added.
...
`ugrid/engines/`, Google docstrings, tests in `tests/ugrid/`, a changelog line, and a reference-doc
stub. Input dataset is never mutated. `location`/`units`/`nodata`/`standard_name` are preserved on
every derived variable (§S.3).
