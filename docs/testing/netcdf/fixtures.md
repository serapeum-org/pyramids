...
| `cf__5v__1d4-4d1__y-asc.nc`                  | CF-1.6    |    5 | 4×1D, 1×4D             | none          | asc    |  ✓  | pyramids-written 4-D cube (round-trip)                                                                                         |
...
| `cf__7v__1d3-2d3-3d1__y-asc.nc`              | CF-1.0    |    7 | 3×1D, 3×2D, 1×3D       | none          | asc    |     | Single CF data var `tos(time,lat,lon)` + coords/bounds                                                                         |
| `cf__8v__1d3-2d3-3d1-4d1__curv-stag.nc`      | CF-1.4    |    8 | 3×1D, 3×2D, 1×3D, 1×4D | none          | —      |  ✓  | Curvilinear + staggered grid                                                                                                   |
...
| `cf__12v__1d4-2d5-3d2-4d1__y-asc.nc`         | CF-1.0    |   12 | 4×1D, 5×2D, 2×3D, 1×4D | none          | asc    |     | Mix of all dimensionalities (CCSM sample): 3-D `pr`/`tas`, 4-D `ua`                                                            |
| `cf__20v__1d3-3d17__y-desc.nc`               | CF-1.0    |   20 | 3×1D, 17×3D            | none          | desc   |  ✓  | Many 3-D vars (17 packed surface fields), ERA-40 subset                                                                        |
| `cf__40v__1d28-2d9-3d3__nc4.nc`              | CF-1.6    |   40 | 28×1D, 9×2D, 3×3D      | none          | —      |  ✓  | Large variable count, netCDF-4, no plain spatial Y                                                                             |
| `cf__48v__1d17-3d21-4d10__y-asc.nc`          | CF-1.0    |   48 | 17×1D, 21×3D, 10×4D    | none          | asc    |  ✓  | CAM init — 10 `(time,lev,lat,lon)` 4-D + 21 3-D                                                                                |
| `coards__4v__1d2-2d2__scaleoffset__y-asc.nc` | COARDS/CF |    4 | 2×1D, 2×2D             | none          | asc    |     | Two vars with `scale_factor`/`add_offset` — packed-data (`unpack=True`) reads                                                  |
| `coards__4v__1d3-3d1__y-desc.nc`             | COARDS    |    4 | 3×1D, 1×3D             | none          | desc   |  ✓  | COARDS `air(time,lat,lon)`, int16-packed; **geographic-descending** orientation case                                           |
| `coards__5v__1d4-4d1__y-desc.nc`             | COARDS    |    5 | 4×1D, 1×4D             | none          | desc   |  ✓  | Single 4-D var `rhum(time,level,lat,lon)`, multi-level                                                                         |
...
mesh files, and files with no spatial Y axis, where a single ascending/descending direction is not meaningful.*
