...
def make_fixed_body_server(body: str, content_type: str, path: str = ""):
    ...
    Shared by the WFS and OGC API – Features tests to avoid duplicating the
    server-spin-up boilerplate. Returns ``(url, counter, httpd)``; the caller is
    responsible for ``httpd.shutdown()`` / ``httpd.server_close()``.
    """
