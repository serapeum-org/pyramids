...
class TestGetTimeVariable:
    ...
    def test_get_time_variable_cds_beta_era5(self):
        ...
        assert valid_time.attrs.get("units") == "seconds since 1970-01-01", (
            f"expected classic units to be merged, got attrs={valid_time.attrs}"
        )
        assert valid_time.attrs.get("calendar") == "proleptic_gregorian", (
        ...
        assert isinstance(result, list), f"expected list, got {type(result)}"
        assert len(result) == valid_time.size, (
...
class TestCubeDimensionNames:
    ...
    def test_cube_dimension_names_matches_container_for_real_4d(self):
        ...
        nc = NetCDF.read_file("tests/data/netcdf/cf__5v__1d4-4d1__geog__y-desc.nc")
        var = nc.get_variable("t")
        assert var.dimension_names == nc.dimension_names, (
