...
def _unlink_best_effort(path: Path) -> None:
    ...
    The ``PermissionError`` arm is Windows-only, and deliberately so rather than by oversight.
    POSIX ``unlink`` on an open file *succeeds*: the name goes, the holder's descriptor keeps
    working, and the inode is freed when the last one closes. So on Linux and macOS a sibling
    worker's sweep does not lose the race — it wins it, removing the sidecar while another
    worker's module-scoped dataset is still using it. That is harmless here only because the
    holder reads through a descriptor it already has, and because the sidecar is regenerated by
    whichever test needs it next. A test that re-opened the raster and asserted the ``.ovr`` were
    on disk would be racy on POSIX and merely warned-about on Windows, which is worth knowing
    before adding one.
    """
    try:
