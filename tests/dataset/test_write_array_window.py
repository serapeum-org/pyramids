...
def blank() -> Dataset:
    ...
    return Dataset.create_from_array(
        np.zeros((5, 5), dtype="float32"), top_left_corner=(0, 5), cell_size=1.0, epsg=4326
    )
...
def blank_multiband() -> Dataset:
    ...
    """
    return Dataset.create_from_array(
        np.zeros((2, 5, 5), dtype="float32"), top_left_corner=(0, 5), cell_size=1.0, epsg=4326
    )
...
class TestWriteArrayWindow:
    ...
    def test_top_left_corner_still_works(self, blank):
        ...
        arr = blank.read_array()
        assert arr[2:4, 2:4].tolist() == [[7.0, 8.0], [9.0, 10.0]], f"tlc patch wrong: {arr}"
    ...
    def test_band_targeted_top_left_corner(self, blank_multiband):
        ...
        """
        blank_multiband.write_array(np.full((2, 2), 5.0), band=0, top_left_corner=[3, 3])
        arr = blank_multiband.read_array()
    ...
    def test_multiband_array_write(self, blank_multiband):
        ...
        arr = blank_multiband.read_array()
        assert arr[0].sum() == 25.0 and arr[1].sum() == 25.0, "Multiband write incomplete"
