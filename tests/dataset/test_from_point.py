...
class TestPointAoiBbox:
    ...
    def test_bbox_brackets_the_point(self):
        ...
        """
        _, (w, s, e, n) = _point_aoi_bbox(46.0, 11.0, edge_size=64, resolution=10.0, units="px")
    ...
    def test_edge_size_px_extent(self):
        ...
        assert abs((e - w) - expected_deg) < 0.1 * expected_deg, f"lon span {(e - w)} vs {expected_deg}"
    ...
    def test_units_metres(self):
        ...
        """
        _, m_bbox = _point_aoi_bbox(0.0, 0.0, edge_size=1000, resolution=10.0, units="m")
        _, px_bbox = _point_aoi_bbox(0.0, 0.0, edge_size=64, resolution=10.0, units="px")
        assert (m_bbox[2] - m_bbox[0]) > (px_bbox[2] - px_bbox[0]), "metres AOI should be wider"
...
class TestFromPoint:
    ...
    def test_composes_search_and_from_stac(self, monkeypatch):
        ...
        assert captured["collection"] == "sentinel-2-l2a", f"collection: {captured['collection']}"
        assert captured["datetime"] == "2021-06-01/2021-06-10", f"datetime: {captured['datetime']}"
        w, s, e, n = captured["bbox"]
        assert w < 11.0 < e and s < 46.0 < n, f"AOI bbox should bracket the point: {captured['bbox']}"
        assert captured["items"] == ["ITEM1", "ITEM2"], "searched items should flow into from_stac"
        assert captured["asset"] == ["B04", "B03"], f"bands should pass through: {captured['asset']}"
    ...
    def test_classmethod_forwards(self, monkeypatch):
        ...
        out = DatasetCollection.from_point(
            46.0, 11.0, collection="c", bands="B04", start_date="2021-01-01",
            end_date="2021-01-31", edge_size=32, resolution=10.0,
        )
        assert out == "CUBE", "classmethod should return the forwarded result"
        assert (seen["lat"], seen["lon"]) == (46.0, 11.0), f"lat/lon not forwarded: {seen}"
        assert seen["kwargs"]["collection"] == "c", f"collection not forwarded: {seen['kwargs']}"
